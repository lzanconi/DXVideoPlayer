    ...
    for (const auto& entry : fs::directory_iterator(folderPath))
    {
        if (entry.is_regular_file() && entry.path().extension() == ".txt")
        {
            ...
            if (filename.find("sequence") != std::string::npos)
            {
                ...
                std::vector<SequenceItem*> sequenceItems = ParseSequenceFile(sequencePath);
...
std::vector<SequenceItem*> ContentManager::ParseSequenceFile(const std::string& sequencePath)
{
    std::cout << ">> Parsing sequence file: " << sequencePath << std::endl;
    ...
    while (std::getline(file, line))
    {
        ...
        sequenceItems.push_back(seqItem);
    }
