...
{
    //If the video source hasn't successfully initialized FFmpeg contexts or hardware devices yet, it skips decoding safely
    if (!isInitialized)
        return true; 

    //Retrieves the standard high-resolution application timer clock value in seconds.
    double currentTime = GetTimeStd();

    //If startTime has not been set via a Play(), the video is considered idle, and it skips reading packets.
    if (startTime <= 0)
        return true;

	//Computes the current playback position. This determines how far into the video we should be.
    double playPos = currentTime - startTime;

    //raw_packet holds compressed data read directly from the container stream.
    AVPacket* raw_packet = av_packet_alloc();
    
    //frame represents the uncompressed raw hardware/D3D11 output frame texture structure generated by the decoder
    AVFrame* frame = av_frame_alloc();

	//Tracks if a fully decoded frame was successfully decoded and copied to the GPU texture during this call.
    bool frameDecoded = false;

	//Verifies if the current playback position has advanced beyond the last decoded frame's timestamp
    if (playPos > lastPTS)
    {
        //Pull the next raw packet from the media file container (fmtCtx) and store it inside raw_packet
        //A retrurn value >= 0 indicates a successful read
        if (av_read_frame(fmtCtx, raw_packet) >= 0)
        {
			//Ensures that the packet belongs to the video stream we're interested in (streamIdx)
            if (raw_packet->stream_index == streamIdx)
            {
				//Sends the compressed packet to the decoder. If the decoder is ready to accept it, it returns 0.
                if (avcodec_send_packet(decCtx, raw_packet) == 0)
                {
					//Tries to receive a fully decoded frame from the decoder. If a complete frame is available, it returns 0.
                    if (avcodec_receive_frame(decCtx, frame) == 0)
                    {
						//Calculates the presentation timestamp (PTS) of the decoded frame
                        internalPTS = frame->best_effort_timestamp * av_q2d(fmtCtx->streams[streamIdx]->time_base);
						//Copy the texture data from the decoded AVFrame to the DirectX 11 texture ready to be rendered on screen
                        CopyFrameToDX11Texture(context, frame);
						//Updates lastPTS to the current position to prevent re-entering this section until the clock ticks forward
                        lastPTS = playPos;
						//Marks that a frame was successfully decoded and copied, allowing the rendering loop to display it.
                        frameDecoded = true;
                        //Captures a high-resolution snapshot timestamp in system nanoseconds of exactly when this frame layout update occurred
        ...
        else
        {
            ...
            {
				//Clean up the allocated frame and packet resources before exiting
                av_frame_free(&frame);
                av_packet_free(&raw_packet);
				//Returns false to signal that the playback has finished
                return false;
            }
        }
    }
...
void VideoSource::StartForcedFadeOut(float duration)
{
