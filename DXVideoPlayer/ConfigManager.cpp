...
void ConfigManager::LoadConfig(const fs::path& filePath)
{
    ...
    if (!fs::exists(resolvedFilename))
    {
        ...
        {
			Logger::LogMessage(MESSAGE_TYPE::INFO, "ConfigManager", "LoadConfig", "Config file '" + filePath.string() + "' not found. Falling back to '" + fallback + "'.");
            resolvedFilename = fallback;
        }
        else
        {
			Logger::LogMessage(MESSAGE_TYPE::INFO, "ConfigManager", "LoadConfig", "Config file '" + filePath.string() + "' not found and fallback '" + fallback + "' also not found. Using defaults.");
			return;
        }
    }
    ...
    try
    {
        ...
        if (j.contains("autorun_id"))
        {
            ...
            if (fs::exists(config.choreos_config_file))
            {
                ...
                {
					Logger::LogMessage(MESSAGE_TYPE::ERRORS, "ConfigManager", "LoadConfig", "Could not resolve autorun_id from choreos config: " + std::string(e.what()));
                }
            }
        }
		...
		Logger::LogMessage(MESSAGE_TYPE::INFO, "ConfigManager", "LoadConfig", ss.str());

    }
    catch (const std::exception& e)
    {
		Logger::LogMessage(MESSAGE_TYPE::ERRORS, "ConfigManager", "LoadConfig", "Error parsing config file: " + std::string(e.what()) + "\n Using defaults.");
    }
}
