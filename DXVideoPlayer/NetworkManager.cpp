...
void NetworkManager::Start()
{
    ...
    if (!clientRunning)
    {
        ...
        Logger::LogMessage(MESSAGE_TYPE::INFO, "NetworkManager", "Start", "CLIENT: Client background thread started");
    }
    ...
    if (!serverRunning)
    {
        ...
        Logger::LogMessage(MESSAGE_TYPE::INFO, "NetworkManager", "Start", "SERVER: Server listener thread started on port " + std::to_string(config.control_port));
    }
}
                ...
                if (percentage >= 1.0)
                {
                    ...
                    if (transitionId >= 0)
                    {
                        ...
                        ss << "TRANSITION PHASE: Transition complete at position " << transition_target_position << "\n"
							<< "Sending back choreography command with parameters: " << std::string(status_buf);

                        Logger::LogMessage(MESSAGE_TYPE::INFO, "NetworkManager", "PositionSend", ss.str());
                        appInterface->HandleNetworkCommand(std::string(status_buf));
                    }
                }
...
void NetworkManager::HandleIncomingConnection(SOCKET clientSocket)
{
    ...
    while (serverRunning)
    {
        ...
        {
            Logger::LogMessage(MESSAGE_TYPE::INFO, "NetworkManager", "HandleIncomingConnection", "SERVER: Client disconnected gracefully");
            break;
        }
        else
        {
            ...
            Logger::LogMessage(MESSAGE_TYPE::ERRORS, "NetworkManager", "HandleIncomingConnection", "SERVER: Client connection lost abruptly.");
            break;
        }
    }
