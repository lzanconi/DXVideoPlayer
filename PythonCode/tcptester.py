...
# FILE_DIRECTORY = r"D:\Workspace\general\videoPlayer\h264" # Use raw string for Windows path
FILE_DIRECTORY = r"./videos" # Use raw string for Windows path

class JSONSenderApp:
    def __init__(self, master):
        self.master = master
        master.title("JSON Command Sender (Silent Mode)")
        master.geometry("600x700") # Increased size to accommodate new controls
        ...
        # Set default filename to the first item if files are found, otherwise empty string
        ...
        self.fade_in_seq_var = tk.StringVar(value="1.0") # <--- ADDED FADE IN FOR SEQUENCE
        self.loop_seq_var = tk.BooleanVar(value=False)      # <--- ADDED LOOP FOR SEQUENCE
        ...
        # UI element for transient feedback (e.g., Send Success/Error)
    ...
    def _load_file_list(self):
        """Reads the filenames from the configured directory."""
        try:
            all_entries = os.listdir(FILE_DIRECTORY)
            ...
            if not file_names:
                # Use print for error, no messagebox
                print(f"Warning: No files found in directory: {FILE_DIRECTORY}")
    ...
    def _create_filename_dropdown(self, parent_frame, row, col, textvariable, file_source):
        ...
        # Check if the list contains an error message (only applies to directory loaded lists)
        if file_source and file_source[0].startswith("No Files Found"):
            # If list contains an error message, display a read-only entry
            ...
            # Create the dropdown menu using the provided file_source list
    ...
    def _create_widgets(self):
        ...
        config_frame = tk.LabelFrame(self.master, text="Target Configuration", padx=10, pady=10)
        ...
        self.connect_button = tk.Button(config_frame, text="Connect", 
                                        command=self._start_connection_thread, width=12)
        self.connect_button.grid(row=1, column=2, padx=5, pady=5)
        
        self.disconnect_button = tk.Button(config_frame, text="Disconnect", 
                                           command=self._disconnect, width=12, state=tk.DISABLED)
        self.disconnect_button.grid(row=1, column=3, padx=5, pady=5)

        # 2. Command Buttons Frame
        commands_frame = tk.LabelFrame(self.master, text="JSON Commands", padx=10, pady=10)
        commands_frame.pack(padx=10, pady=10, fill="x")

        # # --- Command 1: Play Foreground ---
        ...
        # Send Button
        tk.Button(bg_frame, text="Send BG Command", 
                  command=self._send_play_background, width=18).grid(row=3, column=0, columnspan=5, pady=10)

        # --- Command 3: TRANSITION TO ---
        bg_frame = tk.LabelFrame(commands_frame, text="TRANSITION TO", padx=5, pady=5)
        bg_frame.pack(padx=10, pady=10, fill="x")

        # Filename Dropdown
        tk.Label(bg_frame, text="Filename:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self._create_filename_dropdown(bg_frame, 0, 1, self.filename_trans_var, self.file_list) 

        # Fade In
        tk.Label(bg_frame, text="Fade In (s):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        tk.Entry(bg_frame, textvariable=self.fade_in_trans_var, width=10).grid(row=1, column=1, padx=5, pady=5, sticky="w")

        # Fade Out (BG)
        tk.Label(bg_frame, text="Fade Out (s):").grid(row=1, column=2, padx=5, pady=5, sticky="w")
        tk.Entry(bg_frame, textvariable=self.fade_out_trans_var, width=10).grid(row=1, column=3, padx=5, pady=5, sticky="w")

        # FG Fade Out Time (NEW FIELD)
        tk.Label(bg_frame, text="FG Fade Out (s):").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        tk.Entry(bg_frame, textvariable=self.trans_fg_fade_out_time_var, width=10).grid(row=2, column=1, padx=5, pady=5, sticky="w") 
        
        # Loop Checkbox
        tk.Checkbutton(bg_frame, text="Loop", variable=self.loop_trans_var).grid(row=2, column=2, padx=10, pady=5, sticky="w")

        # Send Button
        tk.Button(bg_frame, text="Send Transition Command", 
                  command=self._send_transition_to, width=18).grid(row=3, column=0, columnspan=5, pady=10)


        # --- Command 4: Play Sequence (UPDATED SECTION) ---
        seq_frame = tk.LabelFrame(commands_frame, text="Play Sequence", padx=5, pady=5)
        seq_frame.pack(padx=10, pady=10, fill="x")
        ...
        tk.Button(seq_frame, text="Send Sequence", 
                  command=self._send_play_sequence, width=18).grid(row=2, column=0, columnspan=3, pady=10) # Moved to row 2
    ...
    # --- Utility Methods (Connection and Send Logic) ---
    ...
    def _get_target_info(self):
        ...
        try:
            ...
            self._update_feedback("Input Error: Invalid port number.", is_error=True)
            return None, None
        return ip, port
    ...
    def _update_connection_status(self):
        if self.connected:
            ...
            self._update_feedback("Ready to send commands.")
        else:
    ...
    def _update_feedback(self, message, is_error=False, duration=5000):
        """Updates the status bar feedback message."""
        self.feedback_var.set(message)
        if is_error:
            # Simple way to signal error in the status bar (e.g., change color temporarily)
            self.master.after(100, lambda: self.feedback_var.set(f"ERROR: {message}"))
        
        # Clear the message after a duration (5 seconds)
    ...
    def _start_connection_thread(self):
        ...
        self.status_label.config(text="STATUS: Attempting connection...", fg="orange")
        self.connect_button.config(state=tk.DISABLED)
    ...
    def _connect_target(self, ip, port):
        ...
        try:
            ...
            self.connected = True
            # Connection success is indicated by the status bar changing to GREEN
            ...
            self.connected = False
            self.sock = None
            self.master.after(0, lambda: self._update_feedback(f"Unexpected Connection Error: {e}", is_error=True))
    ...
    def _disconnect(self, silent=False):
        if self.sock:
            try:
                ...
                # Still show socket close warning in the feedback bar, but not a popup
                if not silent:
                    self._update_feedback(f"Socket Warning: Error while closing socket: {e}")
            finally:
                self.sock = None
                self.connected = False
                # Disconnection success is indicated by the status bar changing to RED
    ...
    def _send_json_via_socket(self, message_dict):
        ...
        try:
            ...
            self._disconnect(silent=True) 
            self._update_connection_status()
            self._update_feedback(f"Send Failed: Connection lost ({e}). Reconnect required.", is_error=True)
            return False, ""
        except Exception as e:
            self._update_feedback(f"Send Failed: Unexpected error during send: {e}", is_error=True)
            return False, ""
    ...
    def _validate_float_input(self, var, field_name):
        try:
            ...
            self._update_feedback(f"Input Error: Invalid value for '{field_name}'. Must be a non-negative number.", is_error=True)
            return None
    ...
    def _send_transition_to(self):
        ...
        fade_in = self._validate_float_input(self.fade_in_trans_var, "Background Fade In")
        fade_out = self._validate_float_input(self.fade_out_trans_var, "Background Fade Out")
